---
title: "placenta_hilicdata_cleaning_attempt"
author: "Alana Ferris"
date: "2024-02-02"
...
# Reading in the placenta hilic data
```{r}
placenta_hilic =
  read.table("data/placenta_hilicpos_featuretable.txt", header = TRUE)
  
placenta_hilic_annotation =
  read.csv("data/placenta_hilicpos_annotation_Stage5.csv", header = TRUE) %>% 
  janitor::clean_names()

placenta_hilic_key =
  read.table("data/placenta_mapping_hilicpos.txt", header = TRUE) %>% 
  janitor::clean_names()
```

## placenta_hilic_key: separating info in sample_id column to create a new, clean, sample id called "sid"
```{r}
placenta_hilic_key <-
  placenta_hilic_key %>%
  ...
  filter(file_name%in%names(placenta_hilic))
```

## placenta_hilic: renaming sample ids to the clean sid made in placenta_hilic_key
```{r}
placenta_hilic <- 
  placenta_hilic %>%
  rename_at(vars(placenta_hilic_key$file_name), ~ placenta_hilic_key$sid)
```

## placenta_hilic_annotation: matching annotations
...
placenta_hilic <-
  ...
  left_join(placenta_hilic_annotation %>% 
...
#8232 NAs in annotation_confidence_score but none otherwise 
```

# placenta_hilic: proportion of missing features
```{r}
missing_output <- placenta_hilic %>%
    mutate(missing = rowSums(across(starts_with("S2"), `%in%`, 0))) %>%
    rowwise() %>%
    mutate(prop = missing/sum(grepl("S2",names(placenta_hilic)), na.rm = TRUE)) %>%
...
#ggsave("hilic_placenta_prop_missing.pdf")


met_data_cutoff <- 
  function(cutoffpercent, columnpattern, outputfile){
  met_data_output <- placenta_hilic %>%
...
met_data_50cutoff <- 
  met_data_cutoff(0.5,"S2","placenta_hilic_50cutoff.csv") #14432 obs
```

## placenta_hilic: filter out features where RSD of QC > 30%
...
summary(factor(rsd$threshold)) #2976 below threshold; but i also see i have 5050 NAs...why 
...
met_data_50_rsd_cutoff <- 
  met_data_rsd_cutoff(30,"S2","hilic_50cutoff_RSD.csv") #2876 amount don't pass...this takes a minute to run



```

# placenta_hilic: number of features per sample 
```{r}
met_data_50_rsd_cutoff %>%
...
ggsave("placenta_hilic_analytes_per_sample.pdf")
```

# placenta_hilic: missing value imputation (mice)
```{r}
library(mice)
library(dplyr)
library(patchwork)
...
met_data_replace <-
  ...
  # select(-name, -annotation_confidence_score, -mz_time, -mz, -time, -nist1, -nist2, -q3June2014_1a, -q3June2014_1b, -q3June2014_1c, -q3June2014_1d, -q3June2014_1e, -q3June2014_1f)

#now use this new met_Data_replace to run imputation 
...
init = mice(met_data_replace, maxit=0) 
meth = init$method

#specify cols not to use as predictors 
cols_not_to_use_as_predictors <- c("name", "annotation_confidence_score", "mz_time", 
                                   "mz", "time", "nist1", "nist2", "q3June2014_1a", 
                                   "q3June2014_1b", "q3June2014_1c", "q3June2014_1d", 
                                   "q3June2014_1e", "q3June2014_1f")

#create an empty matrix for non predictors bc that is what function requires 
predictor_matrix <- matrix(NA, nrow = length(cols_not_to_use_as_predictors), 
                           ncol = length(cols_not_to_use_as_predictors),
                           dimnames = list(cols_not_to_use_as_predictors, cols_not_to_use_as_predictors))

# Set the value to 0 for columns not to use as predictors
for (col in cols_not_to_use_as_predictors) {
  predictor_matrix[col, ] <- 0  # Set entire row to 0 for the column
  predictor_matrix[, col] <- 0  # Set entire column to 0 for the column
}

#removing columns you dont want to impute
meth[c("name", "annotation_confidence_score", "mz_time", "mz", "time", "nist1", "nist2", "q3June2014_1a", "q3June2014_1b", "q3June2014_1c", "q3June2014_1d", "q3June2014_1e", "q3June2014_1f")] <- "" 

#creating a vector of columns you DO want to impute 
cols_to_impute <- paste0("S", 2036:21150, "_Placenta")

##applying the cart function across these columns 
for (col in cols_to_impute) {
  meth[[col]] <- "cart"
}

##imputing for only specified columns and predictors 
met_data_impute_2 <- 
  mice(met_data_replace, method = meth, predictorMatrix = predictor_matrix, m = 5)
# not working, saying "length of method differs from number of blocks"
...
#plot distribution of original and imputed data  <- why isnt this working 
densityplot(met_data_impute_2) 
#the original and imputed distribution look similar but tends to overestimate 
...
### placenta_hilic: log transform data
```{r}
met_norm_log2 <- 
  cart_complete_met_data %>%
...
saveRDS(met_log_pareto,"placenta_hilic_50cutoff_RSD_norm_scaled.rds")
```
...
ggsave("placenta_hilic_abundant_annotated.pdf")
```
